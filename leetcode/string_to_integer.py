...
- Do not ignore any characters other than the leading whitespace or the rest of the string after the
  digits.
...
def my_atoi(s: str) -> int:
    ...
    integer = int(prefix + digits) if digits else 0

    if MIN_INT <= integer <= MAX_INT:
        return integer

    return MIN_INT if integer < MIN_INT else MAX_INT
